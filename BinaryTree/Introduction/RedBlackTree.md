...
- A red-black tree is a kind of self-balancing binary search tree. Each node stores an extra bit representing color, used to ensure that the tree remains approximately balanced during insertions and deletions.
  ...
  - Number of black nodes from root to node with less than 2 children is same.

#### 1.1 Rules for Insertion
- Insert to empty tree: create black root node.
- Insert new leaf node as red
  - its parent is black then done.
  - parent is red
      ...
      */
       ...
       */
        ...
        */
         ...
         */
         public boolean validateRedBlackTree(Node root) {
             ...
             if (root.color != Color.BLACK) {
                 System.out.print("Root is no black");
                 return false;
             }
         ...
         private Node insert(Node parent, Node root, int data) {
             ...
             if (root.data > data) {
                 ...
                 // if left becomes root parent means rotation happened at lower level.
                 // So just return left so that nodes at upper level can set their child correctly
                 if (left == root.parent) {
                     return left;
                 }
                 ...
                 // if right becomes root parent means rotation happened at lower level.
                 // So just return right so that nodes at upper level can set their child correctly
             ...
             if (isLeft) {
                 ...
                 if (root.color == Color.RED && root.left.color == Color.RED) {
                    // get the sibling of root. It is returning optional means sibling could be empty
                    ...
                    if (!sibling.isPresent() || sibling.get().color == Color.BLACK) {
                        ...
                        if (isLeftChild(root)) {
                            ...
                            // when right rotatio is done root becomes right child of its left child.
                            // So make root = root.parent because its left child before rotation is new
        ...
        /**
         * Using atomicreference because java does not provide mutable wrapper. Its like double pointer
         * in C.
         */
         private void delete(Node root, int data, AtomicReference<Node> rootReference) {
             ...
             if (root.data == data) {
                 // if node to be deleted has 0 or 1 null children then we have deleteOneChild
                 // use case as discussed in video.
          ...
          */
          private void deleteOneChild(Node nodeToBeDelete, AtomicReference<Node> rootReference) {
              ...
              // replace node with either one not null child if it exits or null child.
              ...
              if (nodeToBeDelete.color == Color.BLACK) {
                  ...
                  if (child.color == Color.RED) {
                      ...
                      // otherwise we have double black situtation.
        ...
        /**
         * If double black node becomes root then we are done. Turning it into single black node
         * just reduces one black in every path.
         */
         ...
         /**
          * If sibling is red and parent and sibling's children are black then rotate it so that
          * sibling becomes black. Double black node is still double black so we need further processing.
          */
        ...
        /**
         * If sibling color is black, parent color is red and sibling's children color is black then 
         * swap color b/w sibling and parent. This increases one black node on double black node path 
         * but does not affect black node count on sibling path. We are done if we hit this situtation.
         */
        ...
        /**
         * If sibling is black, double black node is left child of its parent, siblings right child is
         * black and sibling's left child is red then do a right rotation at siblings left child and swap colors.
         * This converts it to delete case6. It will also have a mirror case.
         */
         private void deleteCase5(Node deleteBlackNode, AtomicReference<Node> rootReference) {
             Node siblingNode = findSiblingNode(doubleBlackNode).get();
         ...
         /**
          * If Sibling is black, double black node is left child of its parent, sibling left child is black and sibling's right child is red,
          * sibling takes its parent color, parent color becomes black,sibling's right child becomes black and then go left rotation at sibling without
          * any further change in color. This removes double black and we are done. This also has a mirror condition.
          */
         ...
         private boolean noRedRedParentChild(Node root, Color parentColor) {
             ...
             return noRedRedParentChild(root.left, root.color) && noRedRedParentChild(root.right, color);
         }
